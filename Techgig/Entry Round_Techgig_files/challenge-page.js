...
Tg_ChallengesPage = new function() {
		...
		$.post(url, {
			'attempt_id': attempt_id
                        ...
                        if(data.status == 'success') {
                            ...
                            $("#coding-platform-head #submit_code input").val("Submit Test");
			}
		});
			...
			$.post(url,{'answer_option':answer_option, 'invitation_id':invitation_id, 'platform_type':platform_type},function(data){
				...
				if(data.length > 0) {
					...
					if(data == 'timeout') {
                        ...
                        if(answer_option == '') {
                            $("#mcq-questions-list .questions-list ul li.current").removeClass('attempted');
                        } else { 
                            $("#mcq-questions-list .questions-list ul li.current").addClass('attempted');
                        }
                    }
				}
			});		
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (msg.status == 'success') {
					...
					$('.questions-progessbar .progress .progress-bar').attr('aria-valuenow', msg.remaining_percentage);
					$('.questions-progessbar .progress .progress-bar').css('width', msg.remaining_percentage+'%');
					if(msg.attempt_question_cnt == 0) {
						$("#check_user_submission").val('N');
					}
                }
            }
        });
	...
	$instance.autoSubmitTest = function(category_key, question_id, encrypt_token, platform_type, isEventPage) {
			...
			if(typeof(encrypt_submit_token) !== 'undefined') {
				encrypt_token = encrypt_submit_token;
			}
			...
			//show alert for submit 
			$(".modal").modal('hide');
			$("#time-elapse-popup").modal({
				keyboard: false
			});
			
			
			window.clearTimeout(SD);
			//after 1 minutes force submit
			setTimeout(function() {
				$('#codejudge_requirement').submit();
			}, 60000); 
...
Jstree = new function () {
    $instance = this;
    $instance.init = function (directory,platform_type,encryptToken,questionCategory,zip_ignore) {
		if( typeof zip_ignore === 'undefined' ) { zip_ignore = ''; }
  
        $('#tree')
                .jstree({
                    'core': {
                        'data': {
                            'url': base_url + "/file_project_question.php?operation=get_node&directory=" + directory + "&cache=" + $.now() + "&ignore=" + zip_ignore,
                            'data': function (node) {
                                return {'id': node.id};
                            }
                        },
        ...
        function saveContentData(directory, fileName, values) {
			...
			if( typeof form_key_post === 'undefined' ) {
				form_key_post = '';
			}
            $.ajax({
                type: "POST",
                url: base_url + '/ajax_files/saas_corporate_function.php?action=saveFileProjectData',
                data: {directory: directory, fileName: fileName, values: values, 'form_key_post':form_key_post},
                dataType: 'json',
                cache: false,
                success: function (data) {
		...
		$('.download-candidate-code').on('click', function () {
            var thisObj = $(this);
			$("<span class='downloading_loader'>Downloading...</span>").insertAfter(thisObj);
            thisObj.hide();

			$.ajax({
                url: base_url + "/ajax_files/saas_corporate_function.php?action=downloadCandidateProject",
                data: {project_dir: directory, invitation_id: encryptToken},
                type: 'POST',
				cache: false,
                success: function (response) {
                    $('.downloading_loader').remove();
					thisObj.show();				
					var download_url = base_url + '/' + directory + '/project.zip';
	...
	$instance.webProjectData = function (projectKey, encrypt_token, buttonCode) {
		$('#firstconsolelogblk').prop("checked",true);
        $('.show-response').html('');
        $('#project-status').html('');
        $('.show-response').hide();
        $('.show-status').show();
        ...
        $.ajax({
            url: base_url + "/ajax_files/saas_candidate_function.php",
            data: {action: 'bind_web_project', buttonCode: buttonCode, projectKey: projectKey, encrypt_token: encrypt_token},
            type: 'POST',
            cache: false,
            success: function (data) {
                $('.disable-before-result input').removeAttr("disabled");
                ...
                if(data.status == 'error') {
					...
					if(data.output != '' && data.output != 'undefined') {
						$('#project-status').html(data.output);
					} else {	
						$('#project-status').html("Please Retry Again!!");
					}
                    return;
                ...
                $('.show-response').html(data.output);
				$('#consolelogblk').prop("checked",true);
				$('#project-status').html('');
                $('.show-response').show();
            }
        });
	...
	$instance.loadWebProjectUrl = function (projectUrl) {
		$("#web-project-iframe").html('<p class="text-center tg-loader mrg40"><img src="' + THEME_PATH + '/images/TG-Loader.gif"></p>');
		checkProjectUrl = setInterval(function(){
			$.ajax({
				url: base_url + "/ajax_files/saas_candidate_function.php",
				data: {action: 'load_web_project', project_url: projectUrl},
				type: 'GET',
				cache: false,
				success: function (data) {
					if(data.status == 'success') {
						clearInterval(checkProjectUrl);
						//setInterval(function(){ Jstree.loadWebProjectIframe(projectUrl); }, 10000); //refresh after every 10 seconds
						Jstree.loadWebProjectIframe(projectUrl);					
					}						
				}	
			});
		}, 5000);  
	...
	$instance.loadWebProjectIframe = function (projectUrl) {
		var iframe = '<iframe src="'+projectUrl+'" scrolling="no" class="posoverlay iframe"></iframe>';
		$("#web-project-iframe").html(iframe);  
    };
};
...
Tg_AssesmentResult = new function () {
	...
	$instance.init = function () {
        ...
        $(".save_question_feedback").click(function () {
            $(".error-msg, .success-msg").remove();
            var seasonId = $("input[name=encrypt_season_id]").val();
            var attempt_id = $("input[name=attempt_id]").val();
            var feedback = $("textarea[name=feedback]").val();
            var rating = $(".practice-ratting ul li.active").length;  
            var action_file_url = base_url + "/";
            var params = {};
            params.feedback = feedback;
            params.rating = rating;
            params.attempt_id = attempt_id;
            var practice = _fetch(API_BASE_URL);
            practice('/techgig/season/' +seasonId+ '/feedback', params).then(function(response) {
                if (response.errors) {
                    throw new Error(response.errors);
                } else {
                    $("<span class='success-msg clearfix'> Saved Successfully!</span>").insertAfter(".save_question_feedback");
                    setTimeout(function(){ $(".error-msg, .success-msg").remove(); }, 3000);
                    $("textarea[name=feedback]").val('')
                }
            })
            .catch(function(error) {
                if(error.responseJSON.errors.rating) {
                    $("<span class='error-msg clearfix'>"+error.responseJSON.errors.rating+"</span>").insertAfter(".save_question_feedback");
                } else if(error.responseJSON.errors.feedback[0]) {
                    $("<span class='error-msg clearfix'>"+error.responseJSON.errors.feedback[0]+"</span>").insertAfter(".save_question_feedback");
                }
                setTimeout(function(){ $(".error-msg, .success-msg").remove(); }, 3000);
            });    
        });
	...
	$instance.showleaderboard = function(level, season_id, level_season_id,flag_one,click_flag) {
        ...
        var action_file_url = base_url + '/ajax_files/load_assessment_leaderboard.php?page=' + page + '&type='+ leaderboard_type + '&season_id='+ season_id +'&level='+level+'&level_season_id='+level_season_id+"&attempt_key="+attempt_key+"&ds_leaderboard="+ds_leaderboard;
        $.ajax({
            type: "POST",
            url: action_file_url,
            data: ({}),
            success: function(response) {
				...
				if(ds_leaderboard == 'public') {
					$("#leaderboard .tabs2 .nav-tabs > li").removeClass('active');
					$("#leaderboard .tabs2 .nav-tabs > li:eq(0)").addClass('active');
				}
				
				if(ds_leaderboard == 'private') {
					$("#leaderboard .tabs2 .nav-tabs > li").removeClass('active');
					$("#leaderboard .tabs2 .nav-tabs > li:eq(1)").addClass('active');
				}
...
Tg_ChallengeDetailPage = new function() {
    var $instance = this;
        ...
        $(document).on('click', '.move-to-editor', function(){
            ...
            setTimeout(function(){ 
				...
				if(typeof monacoEditor !== 'undefined' && monacoEditor != undefined){
				var monacoVal = $('option:selected', '.change-language-confirmation-testcase').attr('monaco_id');
			monacoEditor.dispose();
			//console.log(submittedCode);
			loadEditor(submittedCode,monacoVal);
				}
				else{
	...
	$(window).resize(function() {
		...
		if($("#editor-box").length){
			console.log($("#editor-box").width(),"wd")
...
//			console.log($("#editor-box").width(),"wd")
	...
	$(document).on("click", "#hide-question-area", function() {
		if(typeof monacoEditor !== 'undefined' && monacoEditor != undefined){
			$('#coding-editor').resize(function(){
  monacoEditor.layout();
});	
		}
		
		if($(this).hasClass("less") && questionAreaExpanded == true) {
			questionAreaExpanded = false;
			$(this).removeClass("less");
			$("#question-area").removeClass("hide-contents");
    ...
    $('.get-user-code-diff').trigger('click');
        ...
        $(document).on('click', '.save-compare-change', function(){
			...
			var monacoVal = $('option:selected', '.change-language-confirmation-testcase').attr('monaco_id');
			monacoEditor.dispose();
			loadEditor(modifiedText,monacoVal);				
			}
			else{
		...
		if(typeof monacoEditor !== 'undefined' && monacoEditor != undefined){
			...
			$(".code-compare-elements").val($(".code-compare-elements option:first").val());
            $('#before_merge').hide();
	$('#after_merge').show();
	$('.get-user-code-diff').trigger('click');
            $.ajax({
                    ...
                    cache: false,
                    success: function (data) {
						var rhsData = monacoEditor.getValue();
						if(data.code != ''){
							rhsData = data.code;
						}
                        loadDiffEditor(rhsData);
                    }  
                });
       ...
       // $('#flex-container').css('postion','relative').css('top','auto');
        ...
        $(document).on('change', '.change-language-confirmation-testcase', function(){
            ...
            var newVal = $(this).val();
			var monacoVal = $('option:selected', this).attr('monaco_id');
            ...
            $.ajax({
                type: "POST",
                url: url,
                data: ({'encrypt_token':encrypt_token,'lang_id':lang_id,'evaluationType': evaluationType,'language': newVal}),
                success: function(data) {
					$('#draft_msg').text('');
                    var data = data;
                    if($.trim(data.headTemplate) !=='' || $.trim(data.footTemplate) !==''){
                        ...
                        headditor.setValue($.trim(data.headTemplate), 1);
                        taileditor.setValue($.trim(data.footTemplate), 1)
                        ...
                        var countDefaultTailCodeAfterChange = tailLines.length;
							...
							if(typeof headditor !== 'undefined' && typeof taileditor !== 'undefined'){
							headditor.setValue('', 1);
							taileditor.setValue('', 1);
							}
                            $("#split-editor").hide();
                            $(".tail-split-editor").hide();
                        ...
                        if(data.middleTemplate !=='' && data.middleTemplate){
                            //editor.setValue(data.middleTemplate, 1);
							if(typeof monacoEditor !== 'undefined' && monacoEditor != undefined){
								monacoEditor.dispose();
							loadEditor(data.middleTemplate,monacoVal);
							}
							else{
								editor.setValue(data.middleTemplate, 1);
							}
                        ...
                        if($.trim(data.headTemplate) !=='' || $.trim(data.footTemplate) !==''){
							...
							if (typeof editor.setOption !== 'undefined') {
								editor.setOption("firstLineNumber",  1);
							}
							...
							if (typeof editor.getCursorPosition !== 'undefined') {
								var cursorPos =  editor.getCursorPosition();
								$("#ColsNum").text(cursorPos.column);
							}
                           
							if (typeof editor.getSession !== 'undefined') {
								editor.getSession().selection.on('changeCursor', function(e) {
									var cursorPos =  editor.getCursorPosition();
									$("#RowNum").text(cursorPos.row+1);
									$("#ColsNum").text(cursorPos.column);
                        ...
                        //updateUserCode();
                        $('#auto-complete').trigger('click');
						if(typeof monacoEditor !== 'undefined' && monacoEditor != undefined && evaluation_type==2){
							startLine(monacoEditor);
						}
		...
		$(document).on('click', '#create-new-buffer', function () {
			...
			$("#create-buffer span.buffer-cnt").text(bufferTargetValue);
                ...
                $(document).on('click','#auto-complete',function(){
                   if (typeof editor.session !== 'undefined') {
                ...
                if(typeof editor !== "undefined"){
    				...
    				if(evaluation_type ==2){
                        ...
                        if($.trim(defaultHeadCode)) {
                            ...
                            headditor.setValue($.trim(defaultHeadCode), 1);
                            headditor.getSession().foldAll();
                            $("#split-editor").show();
                        }else{
                            $("#split-editor").hide();
                        ...
                        if($.trim(defaultTailCode)) {
                            ...
                            taileditor.setValue($.trim(defaultTailCode), 1);
                            taileditor.getSession().foldAll();
        ...
        $.ajax({
			...
			error: function (jqXHR, exception) {
				...
				if (jqXHR.status === 0 && exception === 'error') {
					...
					error_text = 'The requested page not found. [404]';
				} else if (jqXHR.status == 500) {
					error_text = 'Internal Server Error [500].';
				} else if (exception === 'parsererror') {
					error_text = 'Requested JSON parse failed.';
				} else if (exception === 'timeout') {
					error_text = 'Time out error.';
				} else if (exception === 'abort') {
					error_text = 'Ajax request aborted.';
				} else {
					error_text = 'Uncaught Error.\n' + jqXHR.responseText;
				}
				$('.nointernetbar').html(error_text);
				$('.nointernetbar').show();
			},
            complete: function() {

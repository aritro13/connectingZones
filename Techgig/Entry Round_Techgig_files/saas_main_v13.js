		...
		success: function(data) {
			...
			if(status == 'exist') {
			  ...
			  //$( "#signup-form .alert-danger").show().html(msg.message); 
				...
				$( "#register_token_form input[type='text']").val("");
				$( "#register_token_form input[type='password']").val("");
			} else {
				window.location.href = window.location.href;
			}		
		}
...
$(document).ready(function(){
	...
	$(document).on('click','.write_warning_close', function(){
		$("#show_allow_new_tab_alert").prop('checked', false);
		...
		current_focus_in_time_stamp = 0;
		
		
		if(webcam){
			//write warning
			$.ajax({url: base_url + '/ajax_files/snap_shot.php', data: {'opt': 'save_log', 'season_id': configWebCam.season_id, 'user_id': configWebCam.user_id, 'attempt_id': configWebCam.attempt_id, 'postdate': 3}, success: function (result) {
			}, type: "POST"});
			
			if(focus_out_seconds > 0) {
				//write warning
				$.ajax({url: base_url + '/ajax_files/snap_shot.php', data: {'opt': 'save_log', 'season_id': configWebCam.season_id, 'user_id': configWebCam.user_id, 'attempt_id': configWebCam.attempt_id, 'postdate': 8, 'focus_out_seconds': focus_out_seconds}, success: function (result) {
			}, type: "POST"});
			}
    ...
    $(document).on('click','#save-ctf-information', function(){
		...
		var cft_value = encodeURIComponent(cft_value);
...
$(document).ready(function(){
    $(document).on('click','#check-ctf-information', function(){
		...
		var	postString = 'question_id='+question_id+'&invitation_id='+invitation_id+'&cft_value='+encodeURIComponent(cft_value);
        var objButton = $(this); 
...
$(document).on('click','#submit_button_subjective', function(){
	...
	var answer_option = $("#text_content").val();
	...
	try{
            ...
            $.post(url,{'answer_option':answer_option,'invitation_id':invitation_id,'form_key_post':form_key_post},function(data){ 
                var msg =data; 
...
$(document).ready(function(){
	...
	$(document).on('click','#render-front-question', function(){
		...
		$.ajax({
			type: "POST",
			url: base_url+'/ajax_files/saas_candidate_function.php?action=Front_end_Submission', 
			data: "html_data="+html_data+"&js_data="+js_data+"&css_data="+css_data+"&question_id="+question_id+"&invitation_id="+invitation_id+"&form_key_post="+form_key_post,
			success: function(data){
				var msg = data; 
				...
				if(msg.status == 'error') {
					...
					if(iframePath.indexOf('?') != -1) {
						iframePath = iframePath+'&current='+current_time;
					} else {
						iframePath = iframePath+'?current='+current_time;
					}
                    $("#Frame").attr('src',iframePath);
                    $("#front-end-render a").attr('href',iframePath);
...
function updateUserCode() {
	...
	try {
		...
		$.post(url,{'encrypt_token':attempt_encrypt_token,'code':code,'save_for_language':save_for_language,'contest_current_link':contest_current_link, interview_contest:interview_contest},function(data) {
				$('#draft_msg').text('Draft saved at '+ formatAMPM(new Date));
			$('#draft_msg').show();
			if(data.length > 0) {
...
function SetDivPosition(){ 
	...
	function updateUserSubjectiveCode() {
        ...
        var subjective_text = $("#text_content").val();
		...
		$.ajax({
			type: "POST",
			url: url,
			data: {'encrypt_token':attempt_encrypt_token, 'code':subjective_text}, // <-- Put comma here
			//contentType: 'text/plain',
			success: function(response){
				var msg = response; 
				if(msg.status == 'success') {
					//saved
					$(".draft_saved").removeClass('hide');
					setTimeout(function(){ 
						$(".draft_saved").addClass('hide');
					}, 2000);
				}
...
function compile_test_execution(action,contest_id,encrypt_token,testcase_type,platform_type,language) {
					...
					var form_key_post = $('#form_key_post').val();							
                      ...
                      if(action == 'submit' && remaining_time > 0 && (form_post != 3)) {
                            SubmitWaitClock();
                      }
					
					if(typeof requestSent === 'undefined') { requestSent = false; }
					  ...
					  $.ajax({
                              type: "POST",
                              url: url, 
                              data: {'code':code,'action':action,'contest_id':contest_id, 'language':language, 'encrypt_token':encrypt_token, 'testcase_type':testcase_type,'own_testcase_input':own_testcase_input,'platform_type':platform_type,'isBotChallenge':isBotChallenge,'candidate_encrypt_token':candidate_encrypt_token, 'form_key_post' : form_key_post},
                              cache: false,
                              timeout: 100000,
                              success: function(data){
                                //return ;
                                                        ...
                                                        if(form_post == 2 && action == 'submit'){
															...
															if(action == 'compile') {
																setTimeout(function(){ 
																		...
																		if($("#editor-box").find(".mCustomScrollbar").length){
																			var target=$(href1).parents(".mCustomScrollbar");
																			target.mCustomScrollbar("scrollTo",href1);
																			$('html, body').animate({scrollTop: $("#editor-main-footer").offset().top - 25}, 1000);	
																			var elmnt = document.getElementById("editor-main-footer");
																			elmnt.scrollIntoView();
																		}else{
																			$('html, body').animate({scrollTop: $("#custom-input-testing").offset().top - 35}, 1000);	
																		}
																}, 800);
															}
	...
	function screen_capture() {
		...
		var headerHightBackup = $("#coding-platform-head").length ? $("#coding-platform-head").offset().top : 0;
		$('body').html2canvas({
        onrendered: function (canvas) {
				...
				//console.log(headerHightBackup, 'aa', $("#coding-platform-head").offset().top);
				headerHightBackup ? $('html, body').scrollTop(headerHightBackup) : '';
				$.ajax({
					type: "POST",
	...
	function userLooseFocus(encrypt_token,question_id,platform_type,out_movement_allowed_number, category_key){
        ...
        if ( document.hasFocus() ) {
            ...
            //valid request 
			if (typeof current_focus_in_time_stamp !== 'undefined') {
			if(current_focus_in_time_stamp < 1) {
				current_focus_in_time_stamp = Math.round((new Date()).getTime() / 1000);
				console.log('reset');
			}
			}
            return true;
			...
			current_focus_out_time_stamp = Math.round((new Date()).getTime() / 1000);
			if (typeof current_focus_in_time_stamp !== 'undefined') {
			if(current_focus_out_time_stamp && current_focus_in_time_stamp) {
				focus_out_seconds = parseInt(current_focus_out_time_stamp) - parseInt(current_focus_in_time_stamp);
			}
			}
			
			if(($("#fullscreenpopup").prop('checked') == false) && ($("#full_screen_escape_alert").prop('checked') == false)){
				$("#show_allow_new_tab_alert").prop('checked', true);
			}
            ...
            if(window_warning >= out_movement_allowed_number){
				...
				//Manoj code : create log for auto submit start
				if(webcam){
					$.ajax({
						url: base_url + '/ajax_files/snap_shot.php',
						data: { 'opt': 'save_log', 'season_id': configWebCam.season_id, 'user_id': configWebCam.user_id, 'attempt_id': configWebCam.attempt_id, 'postdate': 10 },
				...
				//Manoj code : create log for auto submit end
               
			   if(!$("#switching-tabs-not-allowed").is(":visible")){
				   $(".modal").modal('hide');
					$("#switching-tabs-not-allowed").modal('show');
			   }
				
				
				if(webcam){
					//write warning
					$.ajax({url: base_url + '/ajax_files/snap_shot.php', data: {'opt': 'save_log', 'season_id': configWebCam.season_id, 'user_id': configWebCam.user_id, 'attempt_id': configWebCam.attempt_id, 'postdate': 2}, success: function (result) {
					}, type: "POST"});
	...
	function checkUserParticipation(previous_season_id, parent_season_id, season_id) {
		...
		$.get(action_file_url, function(data) {
			...
			if(msg.status == 'success') {
				...
				//$('.msgErrortop .message-box').addClass('info-msg visible-msg').find('p').html(msg.message);
				//$(".message-box").css("right",0);
				$('.common-message-box').addClass("visible-msg");
				$('.common-message-box .message-inner-content').addClass('info-msg').find('p span').html(msg.message);

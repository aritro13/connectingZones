...
var Practice = new function() {
    ...
    if (typeof(API_BASE_URL) != 'undefined') {
        var requester = _fetch(API_BASE_URL);

        $instance.welcomeUser = function() {
            var params = { };
            requester('/techgig/user/welcome', params).then(function(response) {
                    if (response.errors) {
                        throw new Error(response.errors);
                    }
                    $("#skilltest-banner h2.total_coins").text(response);
        ...
        $instance.CalculatePercentage = function(trackId, badge_code) {
            $.ajax({
                type: "POST",
                url: base_url + '/ajax_files/saas_candidate_function.php?action=calculateTrackPercentage',
                data: {
                    'trackId': trackId,
                    'badgeCode': badge_code
                },
                success: function(response) {
                    $("#skilltest-banner .progress-wrap span:first").text(response.percentage + '%');
                    $("#skilltest-banner .progress-wrap .progress-bar span").css('width', response.percentage + '%');
                }
            });
        };

        $instance.skipTutorial = function() {
            return requester(base_url + '/ajax_files/saas_candidate_function.php?action=PracticeSkipTutorial',
            	{ },
        ...
        $instance.redeem = function(id, body) {
        	return requester(
        		"/practice/merchandize/"+id+"/redeem",
        		body,
        		{ method: "POST" }
...
SelectChosen = new function() {
    var $instance = this;
    
    $instance.chosen = function() {
...
Tg_CommonFunction = new function() {
    ...
    var $instance = this;
	/* $instance.showCustomAlert = function(classname, msg,fadeout_time) {
		$(document).find(".save-job-btn,.save-form-btn").addClass("disabled");
		if(typeof(fadeout_time)=='undefined'){
		var fade_out=9000;
		}
		else{
		var fade_out=fadeout_time;
		}
		$(document).find('.common-message-box .message-inner-content').show().removeAttr('class').addClass(classname).addClass('message-inner-content').addClass('clearfix').find('p span').html(msg);
		$(document).find('.common-message-box').show().fadeOut(fade_out,function(){
		$(document).find(".save-job-btn,.save-form-btn").removeClass("disabled");
		});
		$('html, body').animate({
	...
	$instance.showCustomAlert = function(classname, msg, fadeout_time) {
        if (fadeout_time === undefined) {
            fadeout_time = null;
        }
        if (fadeout_time == null || typeof(fadeout_time) == 'undefined') {
            var fade_out = 7000;
        } else {
            var fade_out = fadeout_time;
        }
        fade_out = 7000; //fadeout in 10 seconds
        var popup_html = $(document).find('.common-message-box').html();
        popup_html = '<div class="common-message-box">' + popup_html + '</div>';
        $(document).find('.common-message-box').remove();
        $("body").prepend(popup_html);
        $(document).find('.common-message-box').show().children().show();
        $(document).find('.common-message-box').find('p span').removeAttr('style');
        $(document).find('.common-message-box .message-inner-content').show().removeAttr('class').addClass(classname).addClass('message-inner-content').addClass('clearfix').find('p span').html(msg).fadeOut(fade_out);
        $(document).find('.common-message-box').fadeOut(fade_out);
    };
    ...
    $instance.init = function() {
	 ...
	 $('#coding_faq_submit').on('click', function() {
		...
		$(document).on("click", ".message-box .close", function() {
			$('.common-message-box').removeClass('visible-msg');
		...
		$(document).on("click", ".table1-wrap .expand-collapse-btn-testcase", function() {
			if($(this).parents("tr").hasClass("current")){
				$(this).parents("tr").removeClass("current").next(".open-content").fadeOut();
				$(this).text("View");
			}
				...
				$('.table2 .expand-collapse-btn').text("View");
   ...
   //                      window.clearTimeout(UE);
			...
			if($('#search-form input[type="text"]').val() == ''){
				...
				var page = $('#search-form input[name="page"]').val();
                	...
                	switch(code){
						...
						var max_li = $("#search-form-content .searched-content").find('li').length - 1;						
						var code = e.which;
						switch(code){
							case 40: //down								
								if(liIndex < max_li) { liIndex++; } else { liIndex = 0; }
								$("#search-form-content .searched-content").find('li').removeClass('active');
								$("#search-form-content .searched-content").find('li').eq(liIndex).addClass('active');
								break;
							case 38: //up
								if(liIndex > 0) { liIndex--; } else { liIndex = max_li; }
								$("#search-form-content .searched-content").find('li').removeClass('active');
								$("#search-form-content .searched-content").find('li').eq(liIndex).addClass('active');
								break;
							case 13: //enter
	...
	setTimeout(function(){ $("#sliding-banner").addClass("open"); }, 3000);
		...
		$(document).on("click", "#straight-page-view", function() {
			...
			if($(this).hasClass("less")) {
				...
				$("#editor-box").find(".compare-code-full").css({"width":$("#coding-editor").width()})
        ...
        $("#Subscribe-Email").click(function() {
			...
			var newsletter_chked_response = $('#newsletter_chkbox_agree').is(':checked');
			if(!newsletter_chked_response) {
				$("<span class='error_msg'>You must agree to Terms and Conditions of the Techgig.com</span>" ).insertAfter("#newsletter_block");
				return false;
			} else {
				$("#newsletter-consent").modal('hide');
			}
		...
		$(".social-share-button").click(function() {
			...
			var share_title		=	encodeURIComponent($(this).data('share_title'));
			var share_url		=	encodeURIComponent($(this).data('share_url'));
			var share_content	=	encodeURIComponent($(this).data('share_content'));
			
			var custom_content = $('#'+social_name+'-content').val();
			if(typeof custom_content != 'undefined' && custom_content != '') {
				share_content = encodeURIComponent(custom_content);
			}
			...
			switch(social_name) {
				...
				case 'twitter':
					var url = 'https://twitter.com/intent/tweet?text='+share_content+'&url='+share_url;
					break;	
		...
		$(".follow_user").click(function() {
			...
			var action_source = $(this).data('action_source');
			...
			action_source = (typeof action_source === "undefined") ? "" : action_source;
			...
			$.post(url,{'action_type':action_type, 'follow_val':follow_val, 'encrypt_uid_key':encrypt_uid_key, 'encrypt_webinar_id_key':encrypt_webinar_id_key,'action_source':action_source},function(data) {
				data = trim(data);
				var divs = data.split('##');
				if(data.length > 0) {
					$('#'+tmp_id).text(divs[0]);
		...
		if($("#common-navigation").length > 0 || $("#search-filter").length > 0){
			...
			function hasScrolled() {
				...
				if (st > lastScrollTop && st > navbarHeight){
					...
					$("#header").addClass("fixed-header").removeClass("disabled");//.css("top", 0);
					...
					$("#header").removeClass("disabled");//.css("top", 0);
					$("#header").css({
					   'position' : 'absolute'
					});
	...
	$instance.validateTGInternalEventRegisttrationFrom = function () {   
		...
		if(college=='') {
			$("<span class='error_msg'> Please Select Your College.</span>" ).insertAfter(".intitute-section");
		}
		...
		if(category == ''){
			$("<span class='error_msg'> Please Select Your Participation Category.</span>" ).insertAfter("#category-select");
		}
		...
		if(first_name!='' && email!='' && password!='' && gender != '' && userSkill != '' && college != '' && response.length!=0 && category != '' && chkbox_agree == 1) {
			return true;
		} else {
			return false;
		}
		...
		if(first_name!='' && email!='' && password!='' && chkbox_agree != '' && form_key != '' && either_company_or_college_flag != 'N') {
			...
			$.post(action_url,{action: action_block_name, email: email,first_name:first_name,password:password,sex:sex,reg_source:reg_source,current_company:current_company,reg_type:reg_type,user_skill:token_input_userSkill,form_key:form_key,is_technoutsav:is_technoutsav},function(response) {
					...
					if(data.message=='error') {
						...
						$('#sex').val(sex);
						$('#current_company').val(current_company);
						$("<span class='error_msg'>"+data.email+"</span>" ).insertAfter("#register_email");
	...
	$instance.starRating = function(value,rtnode) {
		rtnode = rtnode || ".rating1";
    ...
    $instance.DataScienceCSV = function () {
        ...
        $(document).on('click', '.submitg-compile-ds', function(){
			...
			var candidate_source_file = $('.file-uploaded-source .file-upload-source').text();
			var candidate_evaluation_file = $('.after-csv-upload .file-upload-csv').text();
			if(candidate_source_file == '' && candidate_evaluation_file == ''){
				$('#evaluation_file_warning').modal('show');
				return false;
			}
			var ignore_source_validation = $(this).attr('data-ignore-source');
			var ignore_source_validation = (typeof ignore_source_validation === 'undefined') ? '' : ignore_source_validation;
			if(candidate_source_file == '' && ignore_source_validation == ''){
				$('#source_file_warning').modal('show');
				return false;
			}
			if(ignore_source_validation == '1'){
				$('#source_file_warning').modal('hide');
			}
            ...
            $(this).text('Submitting..');
          	...
          	$.ajax({
                ...
                data: {'encrypt_token':encrypt_token, 'source_file':candidate_source_file},
                success: function(response){
					$(".submitg-compile-ds"). removeAttr("data-ignore-source");
                    $('.submitg-compile-ds').removeAttr('disabled');
                    $('.submitg-compile-ds').text('Submit & Evaluate');
                    $('.after-csv-upload .file-uploaded').addClass('hide');
	...
	function msg_clear(){		
		$('.common-message-box').removeClass('visible-msg');
		if($('.message-inner-content').hasClass('success-msg,warning-msg,error-msg,info-msg')){
			$('.message-inner-content').removeClass('success-msg').removeClass('warning-msg').removeClass('error-msg').removeClass('info-msg');
			$('.message-inner-content').removeClass('move-top');
		}
	}
	...
	$instance.clearMessage = function () { 
		...
		_msgtimeout = setTimeout(msg_clear, 10002);		
	};
	
	$instance.showAlertMessage = function (msg,alertClass) {     
		alertClass = (typeof alertClass === "undefined") ? "info-msg" : alertClass;
		$(document).find('.common-message-box .message-inner-content').addClass(alertClass).find('p span').text(msg);
        $(document).find('.common-message-box').addClass("visible-msg");
        Tg_CommonFunction.clearMessage();
	};
			...
			if(!err){
				$.post(ajax_url, {user_email:user_email,source_type:source_type, companyUrl:companyUrl, url_val:url_val}, function(data){
					...
					if(data == 'Y') {
						...
						//$('#err_reset_email').show();
						$("<span id='err_reset_email' class='error_msg'>Password reset mail has been sent!!</span>").insertAfter("#reset_email");
						return false;
					} else {
						//
					}
	...
	$instance.loadSkillsSuggestionsDb = function(input_selector,pre_populate_skills) {
		...
		$.getScript(theme_url + "/javascript/jquery.tokeninput.js")
            .done(function() {
				$("#"+input_selector).tokenInput(base_url+"/ajax_files/autocomplete.php?autocomplete=tokeninput&type=technology_tags", {
												theme: "facebook",
												class: "form-control",
												width: 100,
												tabindex: 5,
												...
												suggestedContentUrl: base_url+'/ajax_files/autocomplete.php?autocomplete=tokeninput&type=skills_related_tags',
	...
	$instance.loadSkillsSuggestions = function(input_selector,pre_populate_skills) {
		...
		$.getScript(theme_url + "/javascript/jquery.tokeninput.js")
            .done(function() {
				$.ajax({
					type: "GET",
					url:base_url+'/ajax_files/autocomplete.php?type=technology_tags',
					dataType:'json',
					success: function(data) {
						skill_arr=data;
						$("#"+input_selector).tokenInput(skill_arr, {
							...
							onResult:function (results) {
								...
								if(results.length < 10 && skill_name.length >= 3) {
									$.ajax({				  
										url: base_url+'/ajax_files/autocomplete.php?autocomplete=tokeninput&type=skills_related_tags',
										dataType: "json",
	...
	//This function is used to take consent from user to login via social networks...Sushil 18-Jun-2019
	$(".social-login-button a").click(function() {
		$('#social-login-consent .error_msg').remove();
		$('#social-login-consent #social_chkbox_agree').attr('checked', false);
		
		var provider   = $(this).attr('data-provider');
		var return_url = encodeURIComponent($(this).attr('data-return_url'));
		if(provider != '' && return_url != '') {
			$('#social-login-consent #social_provider').val(provider);
			$('#social-login-consent #social_return_url').val(return_url);
			$('#social-login-consent #social_submit_btn').val('Proceed with '+provider);
			
			$("#TechGigbootStrapModal").modal('hide');
			setTimeout(function(){ $("body").addClass('modal-open'); $("#social-login-consent").modal("show"); }, 500);	
		}
		return false;
	});
	
	//This function is used to login via social networks...Sushil 18-Jun-2019
	$("#social-login-consent #social_submit_btn").click(function() {
		$('#social-login-consent .error_msg').remove();
		var provider	 = $('#social-login-consent #social_provider').val().toLowerCase();
		var return_url	 = $('#social-login-consent #social_return_url').val();
		var chkbox_agree = $('input[name=social_chkbox_agree]:checked').val();
		
		if(chkbox_agree != 1){
			$("<span class='error_msg'>You must agree to the terms and conditions of TechGig before proceeding</span>" ).insertAfter($("#social_chkbox_agree").parent());
			return false;
		}
		
		if(typeof return_url === 'undefined' || return_url == '') {
			return_url = base_url+'%2Fhome';	
		}	
			
		if(provider != '') {	
			//hide social login consent form
			$("#social-login-consent").modal("hide");
		
			if(provider == 'facebook') {
				return fb_login(return_url);
			} else {
				var url;
				switch(provider) {
					case 'google':
						url = base_url+'/social_connect?provider=Google&user_type=external&return_url='+return_url;	
						break;
					case 'linkedin':
						url = base_url+'/linkedin/ln.php?provider=LinkedIn&user_type=external&return_url='+return_url;
						break;					
					case 'github':
						url = base_url+'/social_connect?provider=GitHub&user_type=external&return_url='+return_url;
						break;		
				}				
        ...
        $(document).on('click','.delete-comment', function(){
                ...
                $.ajax({
                        ...
                        success: function() {
								...
								This.closest('.user-comment').html(html);
...
$(function(){
    ...
    //ajax common method API request 
    
    function _fetch(baseURL) {
        return function(url, data, options) {
            return new Promise(function(resolve, reject) {
                ...
                if (window.authToken) {
                    request.headers =  { 'Authorization': 'Bearer ' + window.authToken};
                }
...
/******=========  Code Block Added By Hari  ==============**********/
$(function(){
	/************* Show Hide Control ***************/
	$(document).on("click",'[data-showhide]',function(){
		var _currval = $(this).attr("data-showhidecurr"),_wrpanode = $(this).attr("data-showhide");
		if(_currval == 'false'){
			$(_wrpanode).hide();
		}else{
			$(_wrpanode).show();			
	...
	$(document).on("click",'[data-hideshow]',function(){
		var _currval = $(this).attr("data-showhidecurr"),_wrpanode = $(this).attr("data-hideshow");
		$(_wrpanode).hide();
		$(_currval).show();
	});	
	
	$(document).on("click",'[data-toggle]',function(){
		var _currval = $(this).attr("data-toggle");
		$(_currval).toggle();
	});
	/************* Show Hide Control ***************/
	...
	/************* Max Length and Count Script ***************/
	$(document).on("focus","[data-limitchar]",function(){
		if(!$(this).hasClass("limitactive")){
			var _txtMaxLen = $(this).attr("maxlength") || $(this).attr("data-limitchar");
			$(this).attr("maxlength", _txtMaxLen);
			$(this).attr("data-limitchar", _txtMaxLen);
			$(this).addClass("limitactive").wrap("<div class='max-lengthenable'></div>");
			$(this).after("<span><span class='charcount'>"+textmsgCount($(this))+"</span>/"+ $(this).attr("data-limitchar") +"</span>");
		}
	});
	
	$(document).on("keyup","[data-limitchar]",function(){
		$(this).parents(".max-lengthenable").find(".charcount").text(textmsgCount($(this)));
	});
	/************* ./Max Length and Count Script ***************/
	
	var _indval = getQuerystringNameValue("activetabset") || "";
	/******* Tab Active *********/
	if(!_indval == ""){
		_indval = _indval.replace("active", "")
		if($("#contest-description")){
			//alert("reach")
			var _t = setTimeout(function(){
				$("#contest-description").find("ul.nav-tabs li").eq(_indval).find("a").trigger("click");
			},2000);
		}
		if($("#hackathon-left-menu")){
			var _t = setTimeout(function(){
				$("#hackathon-left-menu").find("ul li").eq(_indval).find("a").trigger("click");
			},2000);		
		}
	}
	/******* ./Tab Active *********/
...
//###############  Common Functions to support above codes
...
function getQuerystringNameValue(name) {
  ...
  if( results == null )
	...
	///count control text length
	function textmsgCount(obj){
		var _tleng = obj.val();
		return _tleng.length;
	}

	function _updatename(obj){
		var _parentObj = $(obj).parents("[data-uploadfield]");
		console.log( $(obj).parents("[data-uploadfield]").length);
...
/******=========  Code Block Ends Written By Hari  ==============**********/

$(document).ajaxComplete(function(event, xhr, options) {
	//$.parseJSON(xhr.responseText);
	//console.log(xhr.responseText);
	var response_val = xhr.responseText;
	if(response_val && typeof response_val != 'undefined' && response_val.indexOf('ForceLogOut') !=-1) {
		response_val = typeof response_val === 'string' ? JSON.parse(response_val) : response_val;
		if(response_val.msg=='ForceLogOut'){
			//location.reload();
			document.location.href = response_val.action_lnk;
		}
	}
});